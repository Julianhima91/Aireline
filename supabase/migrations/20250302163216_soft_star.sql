...
BEGIN
  ...
  RAISE NOTICE 'Processing flight data for % -> % (%), Data found: %', 
  ...
  IF v_flight_data.base_price IS NOT NULL THEN
    ...
    RAISE NOTICE 'Successfully processed flight price: % EUR (Total: % EUR)',
      v_flight_data.base_price,
      v_flight_data.base_price + 20;
  ELSE
    -- Log when no valid flight data is found
    RAISE NOTICE 'No valid flight data found for % -> % (%)',
      NEW.search_params->>'fromCode',
      NEW.search_params->>'toCode',
      v_year_month;
  END IF;
...
Combines both best_flights and other_flights to find the cheapest option.';
