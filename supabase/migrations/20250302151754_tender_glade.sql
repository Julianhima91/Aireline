...
DROP FUNCTION IF EXISTS get_cheapest_flight_from_searches(text, text, text);
...
CREATE INDEX idx_processed_prices_flight_type ON processed_flight_prices(flight_type);
...
CREATE FUNCTION get_cheapest_flight_from_searches(
  p_origin text,
  p_destination text,
  p_year_month text
) RETURNS TABLE (
...
CREATE FUNCTION process_monthly_prices(
  p_origin text,
  p_destination text,
  p_year_month text
) RETURNS void AS $$
...
CREATE FUNCTION process_future_prices(
  p_origin text,
  p_destination text,
...
CREATE FUNCTION get_cheapest_prices(
  p_origin text,
  p_destination text,
  p_months integer DEFAULT 4
) RETURNS TABLE (
