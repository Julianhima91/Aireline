...
CREATE TABLE processed_flight_prices_new (
  ...
  CONSTRAINT processed_prices_route_date_key UNIQUE (origin, destination, flight_date)
);

-- Copy data from old table with date conversion
INSERT INTO processed_flight_prices_new (
  origin,
  destination,
  flight_date,
  base_price,
...
SELECT 
  origin,
  destination,
  year_month || '-01', -- Convert year_month to full date
  base_price,
  commission,
  total_price,
  airline,
...
DROP TABLE processed_flight_prices;
ALTER TABLE processed_flight_prices_new RENAME TO processed_flight_prices;
